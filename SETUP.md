...
Two server-side secrets: the admin token that guards user provisioning
(`/admin`), and the session secret that signs webapp login cookies
(ADR 0010). Per-user credentials are generated by the server and stored
only as hashes (ADR 0005).

```sh
openssl rand -hex 24 | tr -d '\n' | \
  gcloud secrets create podcast-admin-token --data-file=-
openssl rand -hex 32 | tr -d '\n' | \
  gcloud secrets create podcast-session-secret --data-file=-

for s in podcast-admin-token podcast-session-secret; do
  gcloud secrets add-iam-policy-binding $s \
    --member=serviceAccount:${SA} --role=roles/secretmanager.secretAccessor
done
```

The deploy must expose them as `ADMIN_TOKEN` and `SESSION_SECRET`.
Rotating the session secret logs every browser out (API keys and feeds
are unaffected). Read the admin token back when provisioning users:
...
Create an OAuth 2.0 Client ID (type "Web application") in the Google
Cloud console under APIs & Services → Credentials, with the authorized
redirect URI `https://<your-host>/auth/google/callback`. Set
`GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` on the service. Without
them the webapp is password-only and no Google buttons render.
...
Creating a user returns a temporary password exactly once — only hashes
are stored server-side. Log in with it, change it on the Dashboard, and
mint an API key there for publishing:
...
# → {"id":"nico","password":"...","feed_url":"https://.../f/<token>/feed.xml"}
```

Open `${URL}/login`, sign in as `nico`, and create an API key on the
Dashboard (shown once, `pods_…`). Everything below uses it:

```sh
export KEY=<pods_... from the Dashboard>
curl -H "Authorization: Bearer ${KEY}" -X PUT "${URL}/me/image" \
...
```sh
curl -H "Authorization: Bearer ${KEY}" -X DELETE "${URL}/me/episodes/$(date +%F)-test"
```
...
(log in at `${URL}/login`), which also has the reset button should the
URL ever leak.
