...
// TestCreditNamesGeneratorAndHost pins the expanded sign-off end to end:
// when the runner knows its host, the credit the winning engine voices
// names the generating user and that host, not just the voice.
func TestCreditNamesGeneratorAndHost(t *testing.T) {
	st := testStore(t)
	api := newFakeAPI()
	edge := &recordingEngine{name: "edge-tts"}
	r := testRunner(st, api, edge)
	r.host = "radio.nicocesar.com"

	g := newGeneration() // UserID "alice", English
	g.Voice = "female"
	if err := st.PutGeneration(context.Background(), g); err != nil {
		t.Fatal(err)
	}
	...
	want := "Episode generated by alice on radio.nicocesar.com, voiced by Sonia, from Microsoft Edge."
	if got := edge.last(); got != want {
		t.Errorf("credit = %q, want %q", got, want)
	}
}
